...
class EVA(ExternalSolver):
    ...
    def read_results(self,output_path):
        ...
        p = glob.glob(p_string)[0] # NOTE should always exist if finished succesfully, else this returns an error!
        
        f = open(p)
        cost_in_seconds = float(f.readlines()[1]) # seconds are on second line, without text

        return max_acc, cost_in_seconds
    ...
    def inspect_results(self,output_path):
        ...
        # moving averages
        ax2.plot(x,df.rolling(5, min_periods=1).mean().iloc[:,1],label="Simple Moving average over 5 timesteps")
        ax2.plot(x,df.ewm(alpha=0.3, adjust=False).mean().iloc[:,1],label="Exponential Moving Average, alpha = 0.3")
    ...
    def solve(self, X, refinement = None):
        ...
        print("--- Finished sampling batch in {:.2f} minutes ---".format((time.time() - start_time)/60))
